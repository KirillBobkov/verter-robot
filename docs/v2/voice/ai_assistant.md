...
Любое исключение логируется с traceback (`logger.error` + `traceback.format_exc()`), публикуется fallback-ответ «Сервис временно недоступен.» в `text_to_speech`. Retry отсутствует — один запрос, при ошибке сразу fallback.
