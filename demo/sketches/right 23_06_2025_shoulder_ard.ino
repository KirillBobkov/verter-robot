...
// --- ПИНЫ ---
const int ELBOW_UP_PIN = 28;
const int ELBOW_DOWN_PIN = 29;
const int SHOULDER_SERVO_PIN = 39;    // Серво для поворота левого плеча
const int SHOULDER_LIFT_PIN = 8;      // Серво для подъема/опускания левого плеча
...
const int LIFT_UP = 108;            // Вращение для подъема
const int LIFT_DOWN = 91;         // Вращение для опускания
...
void initializeSystem() {
  ...
  Serial.println("Плечо откалибровано в целевое положение");
...
void processCommand(String command) {
  ...
  String action;
  int value;
  parseCommand(command, action, value);
...
void executeCommand(String action, int value) {
  if (action == CMD_ELBOW_UP) {
    ...
    rotateShoulder(value, false);
  }
  else if (action == CMD_SHOULDER_ROTATE_IN) {
    rotateShoulder(value, true);
  }
