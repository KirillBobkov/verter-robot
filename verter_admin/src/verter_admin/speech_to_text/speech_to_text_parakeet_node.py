#!/usr/bin/env python3
"""ROS2 узел для распознавания речи с использованием sherpa-onnx библиотеки и модели Parakeet TDT"""
...
@dataclass
class SpeechToTextParakeetConfig:
    """Конфигурация Speech-to-Text распознавателя с sherpa-onnx и моделью Parakeet TDT"""
    MODEL_DIR: str = 'sherpa-onnx-nemo-parakeet-tdt-0.6b-v3-int8'
    ENCODER_NAME: str = 'encoder.int8.onnx'
    DECODER_NAME: str = 'decoder.int8.onnx'
    JOINER_NAME: str = 'joiner.int8.onnx'
    ...
    NUM_THREADS: int = 4
    FEATURE_DIM: int = 80  # Parakeet TDT использует 80 mel bins
...
class SpeechToTextParakeetNode(Node):
    """ROS2 узел для преобразования речи в текст через sherpa-onnx библиотеку с моделью Parakeet TDT"""
    
    def __init__(self) -> None:
        super().__init__('speech_to_text_parakeet_node')
        
        # Конфигурация
        self.config = SpeechToTextParakeetConfig()
        ...
        self.get_logger().info("🎤 SpeechToTextParakeetNode запущен (sherpa-onnx + Parakeet TDT)")
        ...
        self.get_logger().info(f"   Feature dim: {self.config.FEATURE_DIM}")
    ...
    def _load_models(self) -> None:
        """Загрузка моделей Transducer ASR Parakeet TDT (sherpa-onnx) и VAD"""
        # Определяем пути
        ...
        if not all([encoder_path, decoder_path, joiner_path, tokens_path, vad_model_path]):
            raise RuntimeError("Не найдены файлы моделей sherpa-onnx Parakeet TDT")
        
        # Проверяем метаданные перед загрузкой
        self.get_logger().info("🔄 Проверка метаданных модели Parakeet TDT...")
        metadata_ok = self._check_decoder_metadata(decoder_path)
        if metadata_ok is False:
            self.get_logger().warn(
                "\n⚠️  Проблема с метаданными модели. Попытка загрузки все равно будет выполнена...\n"
            )
        
        # Загрузка распознавателя sherpa-onnx
        self.get_logger().info("🔄 Загрузка Transducer модели Parakeet TDT (sherpa-onnx)...")
        try:
            ...
            self.get_logger().info("✓ Transducer модель Parakeet TDT загружена (sherpa-onnx)")
        except Exception as e:
            error_msg = str(e)
            if "vocab_size" in error_msg.lower() or "metadata" in error_msg.lower():
                self.get_logger().error(
                    ...
                    "Попробуйте изменить feature_dim или другие параметры"
                )
            raise
    ...
    def shutdown(self) -> None:
        ...
        try:
            self.get_logger().info("🔄 Завершение работы SpeechToTextParakeetNode...")
            self.shutdown_event.set()
            self._cleanup_resources()
            self.get_logger().info("✅ SpeechToTextParakeetNode завершен")
        except Exception as e:
...
def main(args=None) -> None:
    ...
    try:
        node = SpeechToTextParakeetNode()
        print("🎤 SPEECH-TO-TEXT NODE (sherpa-onnx + Parakeet TDT)")
