...
class SpeechToTextGigaAMV3CTCNode(Node):
    ...
    def _log_startup_info(self) -> None:
        ...
        self.get_logger().info(f"🎤 SpeechToTextGigaAMV3CTCNode (GigaAM v3 CTC INT8 - {mode_str})")
    ...
    def _load_models(self) -> None:
        ...
        if self.config.USE_CUDA and 'CUDAExecutionProvider' in available_providers:
            ...
            if self.config.USE_CUDA:
                self.get_logger().warn(f"CUDA не найдена. Использую CPU. Доступно: {available_providers}")

        try:
            self.asr_model = ort.InferenceSession(asr_path, sess_options=sess_opts, providers=providers)
            self.get_logger().info(f"✓ GigaAM v3 CTC loaded ({self.asr_model.get_providers()[0]})")
    ...
    def _run_ctc_inference(self, features: np.ndarray) -> str:
        x = np.expand_dims(features.T, axis=0)  # [1, Features, Time]
