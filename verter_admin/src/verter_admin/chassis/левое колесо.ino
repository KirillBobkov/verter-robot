...
void loop() {
  ...
  totalSteps -= diff;  // Инвертирован знак
  prevAngle = angle;
...
void processMovementCommand(String command, bool forward) {
  ...
  // Устанавливаем направление (только правое колесо с энкодером)
  if (forward) {
    currentPWM2 = -FIXED_PWM;  // Инвертирован знак для движения вперед
...
void updateMotorSpeeds() {
  motor1.setSpeed(0);  // Левое колесо отключено
