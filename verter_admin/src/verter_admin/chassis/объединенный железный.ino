...
void loop() {
  ...
  while (Serial.available()) {
    ...
    if (semicolonPos != -1) {
      ...
      processCommand(command.substring(0, semicolonPos));
      // Обрабатываем вторую команду
      processCommand(command.substring(semicolonPos + 1));
    } else {
...
void processWheelCommand(String command, WheelControl* wheel, bool forward, int motorNum) {
...
void checkWheelProgress(WheelControl* wheel, int motorNum) {
  ...
  // Плавное торможение - последние 40 см
  if (remainingDistance <= 0.40 && remainingDistance > 0) {
    float brakeFactor = remainingDistance / 0.40; // от 1.0 до 0.0
...
// Остановка колеса
void stopWheel(WheelControl* wheel, int motorNum) {
  wheel->currentPWM = 0;
  wheel->isMoving = false;
  updateMotorSpeeds();
}
